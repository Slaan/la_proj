...
public class SimplifiedGState implements ISimplifiedGState {
  ...
  @Override public String toString() {
    return String.format("  /-\\\t\tId: %1$d\n" + "  |%2$s|\t\tLife > 40: %6$B\n"
        ...
        (life > 40 ? 1 : 0), gameMap.getNearestTavernDirection().toString(),
  ...
  public static String explainState(State state) {
    ...
    if (id.getId() != 0)
      throw new RuntimeException(
          "Unshifted ID, there is still some information left: " + id.getId());
    return String.format("" + "  /-\\\t\tId: %1$d\n" + "  |%2$s|\t\tLife > 40: %6$B\n"
            + "/-+-+-\\\t\tNearest Tavern: %7$s\n" + "|%3$s|#|%4$s|\t\tNearest Mine: %8$s\n"
            + "\\-+-+-/\n" + "  |%5$s|\n" + "  \\-/\n", state.getStateId(), north.getAbbreviation(),
        west.getAbbreviation(), east.getAbbreviation(), south.getAbbreviation(), life,
        tav.toString(), min.toString());
  }

  public int generateGStateId() {
    IdShifter id = new IdShifter();
    // Generate ID from gs and gameMap.
    id.shift(gameMap.getTileFromDirection(currentPos, DirectionType.NORTH).getValueB0(),
        ID_SHIFT_TILE);
    id.shift(gameMap.getTileFromDirection(currentPos, DirectionType.EAST).getValueB0(),
        ID_SHIFT_TILE);
    id.shift(gameMap.getTileFromDirection(currentPos, DirectionType.SOUTH).getValueB0(),
        ID_SHIFT_TILE);
    id.shift(gameMap.getTileFromDirection(currentPos, DirectionType.WEST).getValueB0(),
        ID_SHIFT_TILE);
