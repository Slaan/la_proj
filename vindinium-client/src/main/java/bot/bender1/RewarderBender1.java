...
public class RewarderBender1 implements IRewarder {
  ...
  public int calculateReward(ISimplifiedGState state) {
    ...
    if (reward > 250) {
      logger.warn(String.format("Reward: %d. <death: %d, tavern: %d, mine: %d, kill: %d>", reward,
          (int) deathReward, (int) tavernReward, (int) mineReward, (int) killReward));
    }
  ...
  private void howDidWeDie() {
    if (formerState.getClosestMine().getDistance().equals((Distance.BESIDE))) {
      ...
      if (move.equals(BotMove.EAST) && formerState.getClosestMine().getDirection()
          .equals(DirectionType.EAST)) {
        gameLog.addDeatbByMine();
      } else if (move.equals(BotMove.SOUTH) && formerState.getClosestMine().getDirection()
          .equals(DirectionType.SOUTH)) {
        gameLog.addDeatbByMine();
      } else if (move.equals(BotMove.WEST) && formerState.getClosestMine().getDirection()
          .equals(DirectionType.WEST)) {
        gameLog.addDeatbByMine();
      } else if (move.equals(BotMove.NORTH) && formerState.getClosestMine().getDirection()
          .equals(DirectionType.NORTH)) {
        gameLog.addDeatbByMine();
      }
    } else {
      gameLog.addDeathByEnemy();
    }
  }
  ...
  private boolean checkForKill() {
    ...
    if (!formerState.getGameMap().getTileFromDirection(formerState.getCurrentPos(), dir)
        .equals(TileType.FREE)) {
      endPos = formerState.getCurrentPos();
    } else if (move.equals(BotMove.EAST)) {
      endPos = new GameState.Position(formerState.getCurrentPos().getX() + 1,
          formerState.getCurrentPos().getY());
    } else if (move.equals(BotMove.SOUTH)) {
      endPos = new GameState.Position(formerState.getCurrentPos().getX(),
          formerState.getCurrentPos().getY() + 1);
  ...
  private double calcKillReward() {
    ...
    for (GameState.Hero h : killedHeroes) {
      ...
      for (int i = 0; i < h.getMineCount(); i++) {
        result += RewardConfigBender1.getKillPerMine() * Math
            .pow(RewardConfigBender1.getKillPerMineDiscount(), i);
      }
    }
  ...
  private boolean grabMine() {
    ...
    if (formerState.getClosestMine().getDistance().equals((Distance.BESIDE)) && formerState
        .getCurrentPos().equals(currentState.getCurrentPos())) {
      // our last action has to be in the direction of the enemy hero
      if (move.equals(BotMove.EAST) && formerState.getClosestMine().getDirection()
          .equals(DirectionType.EAST)) {
        result = true;
